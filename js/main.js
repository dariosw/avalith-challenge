Vue.component('multicard',{
	...
	template:`
				<div class="multiCardContainer grid">
					<card v-for="card in filteredArray" :cardInfo="card"></card>
				</div>`
	,computed:{
		filteredArray: function(){
			var self=this;
       		return this.cardArray.filter(function(card){
       			return card.cardDescription.toLowerCase().indexOf(self.searchTerm.toLowerCase())>=0;})
...
Vue.component('card', {
	...
	template: `
				<div class="cardContainer" >
						<div class="cardImage" @click="showPost">
							<img class="cardImage" :src="cardInfo.cardImageUrl">
						</div class="cardImage">
						<div class="cardText">
							<p>
	...
	,methods:{
		showPost(){
			var dataG=this.cardInfo.cardGraph.data;
			var postTitle=this.cardInfo.cardPost.postTitle+" development";
			var bgUrl=this.cardInfo.cardPost.postImageUrl;
			var postDescription=this.cardInfo.cardPost.postDescription;
			const postCtor= Vue.extend(post);
			const postMnt= new postCtor({propsData:{txtPost:postDescription,titlePost:postTitle,bgUrl:bgUrl}}).$mount('.cards');
			document.getElementById('postContainer').style.backgroundColor="white";
...
document.getElementById('logo').addEventListener('click',function(){
	...
	if(document.getElementById('sideBar').classList.contains('enter')){
		document.getElementById('sideBar').classList.remove('enter');	
	}else{
		document.getElementById('sideBar').classList.add('enter');
	}
	});
